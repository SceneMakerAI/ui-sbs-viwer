...
/**
 * 렌더본 MP4 내려받기(PAGES.md §6).
 *
 * 보안 조치:
 *   · S3 URL 을 화면에 심지 않는다 — 요청 시점에 발급한다.
 *   · comp_id 는 추측 가능한 정수라 **서버에서 노출 대상 여부를 재검증**한다(is_sbs 조인).
 *   · presigned TTL 은 짧게(S3_PRESIGN_TTL, 기본 10분).
 *   · 파일명은 **서버가 정한다** — 사용자 입력을 반영하지 않는다.
 *   · 200MB+ 라 서버 프록시 대신 presigned 리다이렉트를 쓴다.
 */
export async function GET(_req: Request, { params }: { params: Promise<{ compId: string }> }) {
  const compId = Number((await params).compId);
  ...
  const filename = `${safeTitle || "clip"}_${compose.compId}.mp4`;

  log.info("내려받기 발급", { compId, vId: compose.vId });
