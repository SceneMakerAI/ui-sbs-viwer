...
**6. 요청은 서버 큐가 순서대로 보낸다 — 편성 1건 · 렌더 1건.**
`lib/server/queue.ts` 의 두 레인(편성/렌더)이 각각 동시 1건씩 실행한다(서로는 독립).
접수는 항상 202 + 티켓이고, 거절은 대기열 포화(레인별 20건, 503 `QUEUE_FULL`)뿐이다.
현황은 `GET /api/queue` — 대기 순번·진행 단계가 여기 있다. 자세한 건 PAGES.md §5.
**단일 프로세스 전제**(큐가 프로세스 메모리에 있다). PM2 클러스터로 띄우면 무력해진다 —
배포는 systemd 단일 프로세스로 고정한다. 재시작하면 대기분은 사라진다(진행 중 렌더만
`t_compose.render_status=1` 로 복원).
