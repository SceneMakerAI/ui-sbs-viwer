// 렌더 상태는 t_compose.render_status 한 컬럼으로만 읽는다.
// NULL 요청된 적 없음 / 1 진행중 / 0 성공 / -1 실패
// 진행중(1)은 서비스 재기동 후에도 DB 에 남으므로,
// 감시가 끊긴 채 남은 값은 다음 조회가 워커에 물어 정정한다(고아 복구).
// 워커 응답이 없으면 정정하지 않고 막는 쪽을 택한다.
